...
pmodel, _mm = build('/tmp/uturn/model/scene.xml')
...
# ---------- contacts ----------
feet_ref = {fid: pdata.oMf[fid].copy() for fid in (lf, rf)}
...
def running_costs():
    ...
    # control reg
    w_u = np.concatenate([np.full(6, 1e6), np.ones(nu - 6)])
    costs.addCost('uReg', crocoddyl.CostModelResidual(state, crocoddyl.ActivationModelWeightedQuad(w_u), crocoddyl.ResidualModelControl(state, nu)), 1e-4)
    ...
    # feet welded via strong placement+velocity costs
    for fid in (lf, rf):
        costs.addCost(f'foot_{fid}', crocoddyl.CostModelResidual(state, crocoddyl.ResidualModelFramePlacement(state, fid, feet_ref[fid], nu)), 1e2)
        costs.addCost(f'footvel_{fid}', crocoddyl.CostModelResidual(state, crocoddyl.ResidualModelFrameVelocity(state, fid, pin.Motion(np.zeros(6)), pin.WORLD, nu)), 1e1)
    return costs
...
dams, iams = [], []
for i in range(N):
    dam = crocoddyl.DifferentialActionModelFreeFwdDynamics(state, actuation, running_costs())
    dams.append(dam); iams.append(crocoddyl.IntegratedActionModelEuler(dam, dt))
damT = crocoddyl.DifferentialActionModelFreeFwdDynamics(state, actuation, terminal_costs())
...
us = [np.zeros(nu)] * N
t0 = time.time()
done = solver.solve(xs, us, 300, False, 1e-9)
